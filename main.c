...
const char *valid_filters[] = {"blur", "soft_blur", "gaus_small_blur", "sharpen", "motion", "gaus_blur", "negative", "random", NULL};
...
void print_usage(const char *program_name)
{
    ...
    printf("Options:\n");
    printf("  --filter=<type>    Filter type (blur, soft_blur, gaus_small_blur, sharpen, motion, gaus_blur, negative, random)\n");
...
void process_image(const ProgramArgs *args, unsigned char *data, int w, int h)
{
    ...
    else
    {
        ...
        {
            mt_convolution(data, w, h, *selected_filter, args->threads_num, MODE_COLUMN, args->block_size);
        }
        else if (strcmp(args->compute_mode, "by_pixel") == 0)
        {
            mt_convolution(data, w, h, *selected_filter, args->threads_num, MODE_PIXEL, args->block_size);
        }
        else if (strcmp(args->compute_mode, "by_grid") == 0)
        {
            mt_convolution(data, w, h, *selected_filter, args->threads_num, MODE_BLOCK, args->block_size);
        }
    }
